...
def carregar_rostos():
    """Carrega os rostos treinados do arquivo"""
...
def processar_imagem_webcam(img_data_url, nome):
    """Processa imagem capturada da webcam"""
...
def processar_arquivo_imagem(file, nome):
    """Processa arquivo de imagem enviado"""
    filename = secure_filename(f"{nome}.jpg")
    path = os.path.join(FOTOS_FOLDER, filename)
    file.save(path)
    return path
...
def extrair_encoding_facial(imagem_path):
...
def treinar_rostos():
    """Treina o modelo com todas as fotos da pasta"""
    rostos_conhecidos = []
    nomes_conhecidos = []
    erros = []
    ...
    for arquivo in os.listdir(FOTOS_FOLDER):
        if arquivo.endswith(tuple(ALLOWED_EXTENSIONS)):
            ...
            encoding = extrair_encoding_facial(caminho)
            if encoding is not None:
                rostos_conhecidos.append(encoding.tolist())
...
def reconhecer_rosto_em_imagem(img_data_url):
    """Reconhece rosto em uma imagem base64"""
...
def cadastrar():
    if request.method == "POST":
        ...
        if img_data_url:
            ...
            path = processar_imagem_webcam(img_data_url, nome)
            ...
            path = processar_arquivo_imagem(file, nome)
            erros = treinar_rostos()
            if erros:
                flash(
...
def reconhecer():
    ...
    if request.method == "POST":
        ...
        resultado = reconhecer_rosto_em_imagem(img_data_url)
        return resultado
